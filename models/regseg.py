...
from models.backbones.modules import conv1x1, ConvBNAct, Activation


class RegSeg(nn.Module):
    def __init__(self, num_class=1, n_channel=3, dilations=None, act_type='relu6'):
...
class DBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, r1=None, r2=None,
        ...
        if stride == 1:
            ...
            self.conv_left = ConvBNAct(split_ch, split_ch, 3, dilation=r1, act_type=act_type)
            self.conv_right = ConvBNAct(split_ch, split_ch, 3, dilation=r2, act_type=act_type)
        else:   # stride == 2
            assert out_channels % g == 0, 'Group width `g` should be evenly divided by out_channels'
            groups = out_channels // g
            self.conv_left = ConvBNAct(out_channels, out_channels, 3, 2, act_type=act_type)
