...
import torch
from models.deeplabv3_training import (CE_Loss, Dice_loss, Focal_Loss, weights_init)
...
def fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, num_classes, \
    ...
    for iteration, batch in enumerate(gen):
        ...
        # illegal_indices = torch.where((labels < 0) | (labels >= 2))[0]
        #
        # if illegal_indices.numel() > 0:
        #     print("Illegal target values found at indices:", illegal_indices)
