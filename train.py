...
from models.condnet import CondNet
...
from models.deeplabv3_training import (get_lr_scheduler, set_optimizer_lr, weights_init)
...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------------------------------------------------------------# 训练分为两个阶段，分别是冻结阶段和解冻阶段。设置冻结阶段是为了满足机器性能不足的同学的训练需求。 冻结训练需要的显存较小，显卡非常差的情况下，可设置Freeze_Epoch等于UnFreeze_Epoch，此时仅仅进行冻结训练。
    #
    ...
    #   此处获得的mAP会与get_map.py获得的会有所不同，原因有二：5
    #   （一）此处获得的mAP为验证集的mAP。
    ...
    # model = UNetV2(n_classes=2, deep_supervision=False, pretrained_path='pretrain/pvt_v2_b2.pth')
